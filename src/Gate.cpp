...
Gate::Gate(const std::string& chip_name): m_gate_settings_ref(*chip_settings::get_instance()) {
    this->m_name.setString(chip_name);
    this->m_name.setFont(FontManager::get_instance()->get_font("Roboto-Regular.ttf"));

    for (int i = 0; i < 2; i++) {
        this->m_InputPins.emplace_back();
        this->m_InputPins[i].setType(0);
    }

    for (int i = 0; i < 1; i++) {
        this->m_outputPins.emplace_back();
        this->m_outputPins[i].setType(1);
    }
...
void Gate::setId(int chip_id) {
    this->m_id = chip_id;
}

void Gate::setType(int chip_type) {
    this->m_type = chip_type;
}

sf::Vector2f Gate::getPos() {
    return this->m_pos;
}

sf::Vector2f Gate::getSize() {
    return this->m_size;
}

sf::Color Gate::getColor() {
    return this->m_color;
}
...
int Gate::getInputPinCount() {
    return this->m_inputPinCount;
}

int Gate::getOutputPinCount() {
